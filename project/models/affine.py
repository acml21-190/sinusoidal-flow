...
class NeuralLinearRegressor(nn.Module):
    def __init__(self, in_dim):
        ...
        self.fc = nn.Linear(in_features=in_dim, out_features=1)

    def forward(self, x, y):
        mean = self.fc(x)
        base_dist = Normal(mean, 1.0)
        log_dens = base_dist.log_prob(y).reshape(-1)
        return y, log_dens

    def predict(self, x):
        return self.fc(x).reshape(-1, 1, 1, 1)
...
class AffineFlowRegressor(AffineFlow):
    def __init__(self, conditioner: str, num_layers: int, use_norm=False, **kwargs):
        super(AffineFlowRegressor, self).__init__(1, 1, 1, conditioner, num_layers, use_norm, **kwargs)
    ...
    def predict(self, x: torch.Tensor) -> torch.Tensor:
        """
        :param x: independent variable of shape (*, D)
        :return: the predicted value of shape (*, 1)
        """
        assert not self.training, "predict() cannot be called during training. "

        z = x.new_full(x.size()[:-1] + self.chw, self.base_dist.mean.item())
        return self.inv_transform(z, cond_var=x).reshape(-1, 1)


class GaussianMixtureFlow(nn.Module):
    def __init__(self, num_comps: int, c: int, h: int, w: int):
        super(GaussianMixtureFlow, self).__init__()

        flow_dim = c * h * w
        self.mean = nn.Parameter(torch.empty(num_comps, flow_dim))
        self.log_std = nn.Parameter(torch.empty_like(self.mean))
        self.mix_weights = nn.Parameter(torch.empty((num_comps,)))
        self.num_comps = num_comps

        self._init_weights()

    def _init_weights(self):
        nn.init.uniform_(self.mean, -5, 5)
        nn.init.zeros_(self.log_std)
        nn.init.zeros_(self.mix_weights)

    def forward(self, x):  # (*, C, H, W)
        x = torch.flatten(x, start_dim=-3)  # (*, CHW)
        mix = Categorical(torch.softmax(self.mix_weights, dim=-1))
        comp = MultivariateNormal(self.mean, torch.diag_embed(self.log_std.exp()))
        gmm = MixtureSameFamily(mix, comp)
        return None, gmm.log_prob(x)
