...
import torch
from sklearn.datasets import make_regression, load_boston, make_swiss_roll, make_circles, make_moons
...
class ToyRegressionData(pl.LightningDataModule):
    def __init__(self, dataset, batch_size):
        super(ToyRegressionData, self).__init__()

        self.dataset = dataset
        self.batch_size = batch_size
        self.dims, self.datasets = None, None

    def setup(self, stage=None):
        if self.dataset == "linear":
            X, y = make_regression(n_samples=5000, n_features=1, noise=100, random_state=0)
            y = y.reshape(-1, 1, 1, 1) / 100
        ...
        self.dims = X_train[0].shape
        self.datasets = {
            "train": TensorDataset(torch.as_tensor(X_train, dtype=torch.float),
                                   torch.as_tensor(y_train, dtype=torch.float)),
            "val": TensorDataset(torch.as_tensor(X_val, dtype=torch.float), torch.as_tensor(y_val, dtype=torch.float)),
            "test": TensorDataset(torch.as_tensor(X_test, dtype=torch.float),
                                  torch.as_tensor(y_test, dtype=torch.float))
        }
